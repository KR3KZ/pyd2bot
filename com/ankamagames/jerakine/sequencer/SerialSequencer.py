...
class SerialSequencer(EventDispatcher, ISequencer):
      ...
      if not self._running:
         ...
         if self._running:
            while len(self._aStep) > 0 and self._running:
               self.execute()
               if self._currentStep and isinstance(self._currentStep, AbstractSequencable) and not self._currentStep:
                  self._running = False
   ...
   def execute(self) -> None:
      ...
      try:
         ...
         if self._defaultStepTimeout != int.MIN_VALUE and self._currentStep.hasDefaultTimeout:
            self._currentStep.timeout = self._defaultStepTimeout
         if self.has_listeners(SequencerEvent.SEQUENCE_STEP_START):
            self.dispatch(SequencerEvent.SEQUENCE_STEP_START, SequencerEvent(self,self._currentStep))
         self._currentStep.start()
      except Exception as e:
         if isinstance(self._currentStep, ISubSequenceSequencable):
            self._activeSubSequenceCount -= 1
            ISubSequenceSequencable(self._currentStep).remove_listener(SequencerEvent.SEQUENCE_END, self.onSubSequenceEnd)
   ...
   def stepFinished(self, step:ISequencable, withTimout:bool = False) -> None:
      ...
      if self._running:
         ...
         if self.has_listeners(SequencerEvent.SEQUENCE_STEP_FINISH):
            self.dispatch(SequencerEvent(SequencerEvent.SEQUENCE_STEP_FINISH,self,self._currentStep))
         worker = EnterFrameDispatcher().worker
         worker.addSingleTreatmentAtPos(self, self.start, [], worker.findTreatments(None, self.start, []).length)
