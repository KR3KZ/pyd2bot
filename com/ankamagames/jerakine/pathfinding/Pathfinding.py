...
class Pathfinding:
   ...
   @classmethod
   def findPath(cls, mapData:IDataMapProvider, start:MapPoint, end:MapPoint, allowDiag:bool = True, bAllowTroughEntity:bool = True, aNoneObstacles:bool = True) -> MovementPath:
      ...
      while(len(cls._openList) > 0 and cls._isCellClosed[endCellId] == False):
         ...
         for y in range(parentY - 1, parentY + 2):
            for x in range(parentX - 1, parentX + 2):
               ...
               if cellId != MapTools.INVALID_CELL_ID and cls._isCellClosed[cellId] == False and cellId != parentId and\
                       mapData.pointMov(x, y, bAllowTroughEntity, parentId, endCellId, aNoneObstacles) and\
                           (y == parentY or x == parentX or allowDiag and\
                               (mapData.pointMov(parentX, y, bAllowTroughEntity, parentId, endCellId, aNoneObstacles) or\
                               mapData.pointMov(x, parentY, bAllowTroughEntity, parentId, endCellId, aNoneObstacles))):
      ...
      while cursor != startCellId: 
         ...
         if allowDiag:
            ...
            grandParent = int(MapTools.INVALID_CELL_ID) if parent == MapTools.INVALID_CELL_ID else int(cls._parentOfCell[parent])
            ...
            if grandParent != MapTools.INVALID_CELL_ID and MapTools.getDistance(cursor, grandParent) == 1:
               ...
               if mapData.pointMov(interX, interY, bAllowTroughEntity, cursor, endCellId) and mapData.pointWeight(interX, interY) < 2:
               ...
               if kX + kY == nextX + nextY and kX - kY != interX - interY and not mapData.isChangeZone(MapTools.getCellIdByCoord(kX, kY),MapTools.getCellIdByCoord(interX, interY)) and not mapData.isChangeZone(MapTools.getCellIdByCoord(interX, interY),MapTools.getCellIdByCoord(nextX, nextY)):
                  cls._parentOfCell[cursor] = grandParent
               elif kX - kY == nextX - nextY and kX - kY != interX - interY and not mapData.isChangeZone(MapTools.getCellIdByCoord(kX, kY),MapTools.getCellIdByCoord(interX, interY)) and not mapData.isChangeZone(MapTools.getCellIdByCoord(interX, interY),MapTools.getCellIdByCoord(nextX, nextY)):
                  cls._parentOfCell[cursor] = grandParent
               elif kX == nextX and kX != interX and mapData.pointWeight(kX, interY) < 2 and mapData.pointMov(kX, interY, bAllowTroughEntity, cursor, endCellId):
                  cls._parentOfCell[cursor] = MapTools.getCellIdByCoord(kX, interY)
               elif kY == nextY and kY != interY and mapData.pointWeight(interX, kY) < 2 and mapData.pointMov(interX, kY, bAllowTroughEntity, cursor, endCellId):
