...
class ItemWrapper(Item, ISlotData, ICellZoneProvider, IDataCenter):
      ...
      LEVEL_STEP:list = [0,10,21,33,46,60,75,91,108,126,145,165,186,208,231,255,280,306,333,361]
      ...
      def create(self, position:int, objectUID:int, objectGID:int, quantity:int, newEffects:list[ObjectEffect], useCache:bool = True) -> 'ItemWrapper':
         item:ItemWrapper = None
         effect:EffectInstance = None
         refItem:Item = Item.getItemById(objectGID)
         cachedItem:ItemWrapper = self._cache[objectUID] if objectUID > 0 else self._cacheGId[objectGID]
      ...
      def createFromServer(self, itemFromServer:ObjectItem, useCache:bool = True) -> 'ItemWrapper':
         ...
         cachedItem:ItemWrapper = self._cache[itemFromServer.objectUID] if itemFromServer.objectUID > 0 else self._cacheGId[itemFromServer.objectGID]
         if not cachedItem or not useCache:
            if refItem.isWeapon:
               item = WeaponWrapper()
            else:
      ...
      def isHarness(self) -> bool:
         return type.id == DataEnum.ITEM_TYPE_HARNESS_MOUNT or type.id == DataEnum.ITEM_TYPE_HARNESS_MULDO or type.id == DataEnum.ITEM_TYPE_HARNESS_FLYHORN
      ...
      def isCertificate(self) -> bool:
         ...
         return itbt and (itbt.typeId == DataEnum.ITEM_TYPE_DRAGOTURKEY_CERTIFICATE or itbt.typeId == DataEnum.ITEM_TYPE_MULDO_CERTIFICATE or itbt.typeId == DataEnum.ITEM_TYPE_FLYHORN_CERTIFICATE)
      ...
      def update(self, position:int, objectUID:int, objectGID:int, quantity:int, newEffects:list[ObjectEffect]) -> None:
      ...
      def updateEffects(self, updateEffects:list[ObjectEffect]) -> None:
         ...
         for effect in updateEffects:
            ...
            if multiUseCheck != -1 and (effectInstance.effectId == ActionIds.ACTION_CHARACTER_ENERGY_POINTS_WIN or effectInstance.effectId == ActionIds.ACTION_CHARACTER_BOOST_LIFE_POINTS or effectInstance.effectId == ActionIds.ACTION_CHARACTER_GAIN_XP or effectInstance.effectId == ActionIds.ACTION_CHARACTER_INVENTORY_GAIN_KAMAS or effectInstance.effectId == ActionIdProtocol.ACTION_CHARACTER_INVENTORY_ADD_ITEM_NOCHECK):
               multiUseCheck = 1
            if multiUseCheck != -1 and effectInstance.effectId != ActionIds.ACTION_CHARACTER_ENERGY_POINTS_WIN and effectInstance.effectId != ActionIds.ACTION_CHARACTER_BOOST_LIFE_POINTS and effectInstance.effectId != ActionIds.ACTION_CHARACTER_GAIN_XP and effectInstance.effectId != ActionIds.ACTION_CHARACTER_INVENTORY_GAIN_KAMAS and effectInstance.effectId != ActionIdProtocol.ACTION_CHARACTER_INVENTORY_ADD_ITEM_NOCHECK and effectInstance.effectId != ActionIds.ACTION_MARK_NEVER_TRADABLE_STRONG and effectInstance.effectId != ActionIds.ACTION_MARK_NEVER_TRADABLE and effectInstance.effectId != ActionIds.ACTION_MARK_NOT_TRADABLE:
               multiUseCheck = -1
            if effectInstance.effectId == ActionIds.ACTION_MARK_NOT_TRADABLE:
               self.exchangeAllowed = False
            if effectInstance.effectId == ActionIds.ACTION_MARK_NEVER_TRADABLE_STRONG or effectInstance.effectId == ActionIds.ACTION_MARK_NEVER_TRADABLE:
               exchangeable = False
            if effectInstance.effectId == ActionIds.ACTION_ITEM_WRAPPER_COMPATIBLE_OBJ_TYPE:
