...
class EntityStats:
   ...
   def getHealthPoints(self) -> float:
      return self.getMaxHealthPoints() + self.getStatTotalValue(StatIds.CUR_LIFE) + self.getStatTotalValue(StatIds.CUR_PERMANENT_DAMAGE)
   ...
   def getMaxHealthPoints(self) -> float:
      ...
      if isinstance(vitalityStat, DetailedStat):
         ...
         effectiveVitality = max(0,detailedVitalityStat.baseValue + detailedVitalityStat.objectsAndMountBonusValue + detailedVitalityStat.additionalValue + detailedVitalityStat.alignGiftBonusValue) + detailedVitalityStat.contextModifValue
      elif isinstance(vitalityStat, Stat):
         effectiveVitality = vitalityStat.totalValue
      return self.getStatTotalValue(StatIds.LIFE_POINTS) + effectiveVitality - self.getStatTotalValue(StatIds.CUR_PERMANENT_DAMAGE)
