      ...
      for spellWrapper in spellList:
         if spellWrapper.spellLevelInfos.initialCooldown is not 0:
            ...
            if self._spells[spellWrapper.spellId] == None:
               self._spells[spellWrapper.spellId] = SpellManager(self,spellWrapper.spellId,spellWrapper.spellLevel)
   ...
   def updateCooldowns(self, spellCooldowns:list[GameFightSpellCooldown] = None) -> None:
      ...
      for k in range(numCoolDown):
         ...
         if spellW and spellW.spellLevel > 0:
            ...
            spellCastManager.getSpellManagerBySpellId(spellW.id).forceLastCastTurn(self.currentTurn + spellCooldown.cooldown - interval)
      self.needCooldownUpdate = False
   
   def castSpell(self, pSpellId:int, pSpellLevel:int, pTargets:list, pCountForCooldown:bool = True) -> None:
      if self._spells[pSpellId] == None:
         self._spells[pSpellId] = SpellManager(self,pSpellId,pSpellLevel)
      self._spells[pSpellId]
   
   def getSpellManagerBySpellId(self, pSpellId:int, isForceNewInstance:bool = False, pSpellLevelId:int = -1) -> SpellManager:
