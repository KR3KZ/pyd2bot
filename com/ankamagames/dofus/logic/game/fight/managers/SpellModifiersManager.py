...
class SpellModifiersManager(metaclass=Singleton):
   ...
   def __init__(self):
      ...
      if _dataStoreType is None:
         _dataStoreType = DataStoreType(self.DATA_STORE_CATEGORY,True,DataStoreEnum.LOCATION_LOCAL,DataStoreEnum.BIND_COMPUTER)
      rawIsVerbose = StoreDataManager().getData(_dataStoreType,self.DATA_STORE_KEY_IS_VERBOSE)
   ...
   def setRawSpellsModifiers(self, entityId:float, rawSpellsModifiers:list[CharacterSpellModification]) -> None:
      ...
      if rawSpellsModifiers is not None and len(rawSpellsModifiers) > 0:
         ...
         for rawSpellModifier in rawSpellsModifiers:
            ...
            spellModifier = SpellModifier(rawSpellModifier.modificationType,rawSpellModifier.value.base,rawSpellModifier.value.additional,rawSpellModifier.value.objectsAndMountBonus,rawSpellModifier.value.alignGiftBonus,rawSpellModifier.value.contextModif)
   ...
   def setRawSpellModifier(self, entityId:float, rawSpellModifier:CharacterSpellModification) -> None:
      ...
      spellModifier:SpellModifier = SpellModifier(rawSpellModifier.modificationType,rawSpellModifier.value.base,rawSpellModifier.value.additional,rawSpellModifier.value.objectsAndMountBonus,rawSpellModifier.value.alignGiftBonus,rawSpellModifier.value.contextModif)
      spellModifiers.setModifier(spellModifier)
   
   def deleteSpellsModifiers(self, entityId:float) -> bool:
      key:str = str(entityId)
      if key not in self._entitiesMap:
         logger.error("Tried to del spells modifier stats for entity with ID " + key + ", but none were found. Aborting")
         return False
   ...
   def deleteSpellModifiers(self, entityId:float, spellId:float) -> bool:
      ...
      spellKey:str = str(spellId)
      if entityKey not in self._entitiesMap:
         logger.error("Tried to del spell " + spellKey + " modifiers for entity with ID " + entityKey + ", but no spells modifier stats were found. Aborting")
         return False
      spellModifiers:SpellModifiers = self._entitiesMap[entityKey]
      if not spellModifiers or spellKey not in spellModifiers:
         logger.error("Tried to del spell " + spellKey + " modifiers for entity with ID " + entityKey + ", but none were found. Aborting")
         return False
