...
from com.ankamagames.dofus.network.messages.game.actions.GameActionAcknowledgementMessage import GameActionAcknowledgementMessage
...
from com.ankamagames.dofus.network.messages.game.context.fight.GameFightTurnReadyMessage import GameFightTurnReadyMessage
...
class FightBattleFrame(Frame):
   ...
   def process(self, msg:Message) -> bool:
      ...
      if isinstance(msg, GameFightTurnListMessage):
         ...
         if self._executingSequence or self._currentSequenceFrame:
            logger.warn("There was a turns list update during self sequence... Let\'s wait its finish before doing it.")
      ...
      if isinstance(msg, GameFightSynchronizeMessage):
         ...
         if self._newWave:
            for fighter in gftcimsg.fighters:
               if fighter.spawnInfo.alive and fighter.wave == self._newWaveId and not FightEntitiesFrame.getCurrentInstance().getEntityInfos(fighter.contextualId):
         ...
         if self._executingSequence:
            ...
            self._synchroniseFightersInstanceId = FightSequenceFrame.currentInstanceId
         else:
      ...
      if isinstance(msg, GameActionUpdateEffectTriggerCountMessage):
         ...
         for effectTrigger in gauetcmsg.targetIds:
            for buffTriggered in BuffManager().getAllBuff(effectTrigger.targetId):
               if buffTriggered is TriggeredBuff and buffTriggered.effect.effectUid == effectTrigger.effectId:
      ...
      if isinstance(msg, SlaveSwitchContextMessage):
         ...
         if sscmsg.masterId == playerId:
            ...
            if not self._currentPlayerId and self._turnsList.find(self._masterId) > self._turnsList.find(self._slaveId):
      ...
      if isinstance(msg, GameFightTurnStartMessage):
         ...
         if not isResumeMessage:
            ...
            if self._slaveId and self._currentPlayerId != currentPlayedFighterId and (self._slaveId == self._currentPlayerId or nextPlayable == self._slaveId):
               self.prepareNextPlayableCharacter(self._masterId)
         if gftsmsg.id > 0 or self._playingSlaveEntity:
            if FightEntitiesFrame.getCurrentInstance().getEntityInfos(gftsmsg.id) and FightEntitiesFrame.getCurrentInstance().getEntityInfos(gftsmsg.id).disposition.cellId != -1 and not FightEntitiesHolder().getEntity(gftsmsg.id):
               ...
               if entity != None:
                  ...
                  ss2 = SerialSequencer()
                  ss2.addStep(AddGfxEntityStep(153,entity.position.cellId,0,-yOffset))
                  ss2.start()
               self._playerNewTurn = entity
         deadEntityInfo = FightEntitiesFrame.getCurrentInstance().getEntityInfos(gftsmsg.id)
         if (gftsmsg.id == playerId or self._playingSlaveEntity) and deadEntityInfo and deadEntityInfo.spawnInfo.alive:
         ...
         if self._turnFrame.myTurn and PlayerManager().kisServerPort > 0:
            if ExternalNotificationManager().canAddExternalNotification(ExternalNotificationTypeEnum.KOLO_TURN_START):
               pass
      ...
      if isinstance(msg, GameFightTurnStartPlayingMessage):
            SpeakingItemManager().triggerEvent(SpeakingItemManager.SPEAK_TRIGGER_PLAYER_TURN_START)
            return True
      ...
      if isinstance(msg, SequenceEndMessage):
         ...
         if not self._currentSequenceFrame:
            ...
            return True
         self._currentSequenceFrame.mustAck = semsg.authorId == CurrentPlayedFighterManager().currentFighterId
      ...
      if isinstance(msg, GameFightLeaveMessage):
         gflmsg = msg
         fighterInfos2 = FightEntitiesFrame.getCurrentInstance().getEntityInfos(self._lastPlayerId)
   ...
   def pulled(self) -> bool:
      ...
      if self._autoEndTurnTimer:
         self._autoEndTurnTimer.removeEventListener(TimerEvent.TIMER_COMPLETE,self.sendAutoEndTurn)
   ...
   def prepareNextPlayableCharacter(self, currentCharacterId:float = 0) -> None:
      ...
      if self._slaveId:
         if currentCharacterId:
            nextCharacterId = float(self._masterId) if currentCharacterId == self._slaveId else float(self._slaveId)
         else:
            nextCharacterId = self.getNextPlayableCharacterId()
         nextCharacterEntity = FightEntitiesFrame.getCurrentInstance().getEntityInfos(nextCharacterId)
   ...
   def waitAnimations(self) -> None:
      ...
      for index in range(len(entityIdList)):
         ...
         if tiphonSprite is not None:
            if tiphonSprite.isPlayingAnimation() and not tiphonSprite.isCurrentAnimationStatic():
      ...
      if tiphonSpriteToListen is not None:
         tiphonSpriteToListen.addEventListener(TiphonEvent.ANIMATION_END,self.onLastAnimationFinished)
      else:
         self.sendAcknowledgement()
   
   def onLastAnimationFinished(self, tiphonEvent:TiphonEvent = None) -> None:
      tiphonEvent.sprite.removeEventListener(TiphonEvent.ANIMATION_END,self.onLastAnimationFinished)
      self.sendAcknowledgement()
   ...
   def sendAcknowledgement(self) -> None:
      ...
      ack.initGameActionAcknowledgementMessage(True,self._sequenceFrameCached.ackIdent)
   ...
   def finishSequence(self, sequenceFrame:FightSequenceFrame) -> FunctionType:
      def function() -> None:
         ...
         if self._refreshTurnsList:
            ...
            self._newTurnsList = None
            self._newDeadTurnsList = None
         if not self._executingSequence and len(self._sequenceFrames) and self._sequenceFrames[0].instanceId >= self._synchroniseFightersInstanceId:
   ...
   def confirmTurnEnd(self) -> None:
      entityId:float = None
      fighterInfos2:GameFightFighterInformations = FightEntitiesFrame.getCurrentInstance().getEntityInfos(self._lastPlayerId)
      if fighterInfos2:
         ...
         pass
      spellCastManager:SpellCastInFightManager = CurrentPlayedFighterManager().getSpellCastManagerById(self._lastPlayerId)
   ...
   def gameFightSynchronize(self, fighters:list[GameFightFighterInformations]) -> None:
      ...
      for fighterInfos in fighters:
         ...
         if fighterInfos.spawnInfo.alive:
            newWaveMonster = fighterInfos.wave == self._newWaveId and fighterInfos.wave != 0 and not DofusEntities.getEntity(fighterInfos.contextualId)
            ...
            if newWaveMonster:
               ...
               sequencer.addStep(AddGfxEntityStep(2715, fighterInfos.disposition.cellId))
               sequencer.addStep(FightVisibilityStep(fighterInfos.contextualId, True))
               sequencer.start()
