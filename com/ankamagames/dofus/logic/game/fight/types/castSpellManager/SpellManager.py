...
class SpellManager:
   ...
   def __init__(self, spellCastManager:SpellCastInFightManager, pSpellId:int, pSpellLevel:int):
      super().__init__()
   ...
   def cooldown(self) -> float:
      ...
      if interval == 63:
         return 63
      initialCooldown:int = self._lastInitialCooldownReset + spellLevel.initialCooldown - self._spellCastManager.currentTurn
      if self._lastCastTurn >= self._lastInitialCooldownReset + spellLevel.initialCooldown or spellLevel.initialCooldown == 0 or self._forcedCooldown or self._castThisTurn > 0 or self._spellHasBeenCast:
   ...
   def forceCooldown(self, cooldown:int, isBonusRefresh:bool = False) -> None:
      ...
      spellW:SpellWrapper = SpellWrapper.getSpellWrapperById(self._spellId,self._spellCastManager.entityId)
      if isBonusRefresh:
   ...
   def updateSpellWrapper(self) -> None:
      spellW:SpellWrapper = SpellWrapper.getSpellWrapperById(self._spellId,self._spellCastManager.entityId)
      if spellW == None:
         spellW = SpellWrapper.create(self._spellId,self._spellLevel,True,self._spellCastManager.entityId)
