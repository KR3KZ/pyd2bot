...
class FightEventsHelper:
   ...
   SKIP_ENTITY_ALIVE_CHECK_EVENTS:list = [FightEventEnum.FIGHTER_GOT_KILLED,FightEventEnum.FIGHTER_DEATH]
   ...
   def addFightText(self, fightEvent:FightEvent) -> None:
      ...
      if fightEvent.name == FightEventEnum.FIGHTER_LIFE_LOSS or fightEvent.name == FightEventEnum.FIGHTER_LIFE_GAIN or fightEvent.name == FightEventEnum.FIGHTER_SHIELD_LOSS:
         fightEvent.params.append(self._lastSpellId)
      if fightEvent.name == FightEventEnum.FIGHTER_LIFE_LOSS:
         for i in range(num):
            event = self._events[i][0]
            if event.name == FightEventEnum.FIGHTER_REDUCED_DAMAGES and event.castingSpellId == fightEvent.castingSpellId and event.targetId == fightEvent.targetId:
               groupByType = False
      if groupByType:
         for i in range(num):
            eventList = self._events[i]
            event = eventList[0]
            if event.name == fightEvent.name and (event.castingSpellId == fightEvent.castingSpellId or fightEvent.castingSpellId == -1):
               if (event.name == FightEventEnum.FIGHTER_LIFE_LOSS or fightEvent.name == FightEventEnum.FIGHTER_LIFE_GAIN or fightEvent.name == FightEventEnum.FIGHTER_SHIELD_LOSS) and event.params[len(event.params) - 1] != fightEvent.params[len(fightEvent.params) - 1]:
                  break
      ...
      while self._events:
         ...
         if eventList == None or len(eventList) == 0:
            ...
            for i in range(numTargets):
               ...
               if eventsGroupedByTarget[targetEvents].length > 1 and (eventBase.name == FightEventEnum.FIGHTER_LIFE_LOSS or eventBase.name == FightEventEnum.FIGHTER_LIFE_GAIN or eventBase.name == FightEventEnum.FIGHTER_SHIELD_LOSS):
                  if eventBase.name  == FightEventEnum.FIGHTER_LIFE_LOSS or eventBase.name  == FightEventEnum.FIGHTER_SHIELD_LOSS:
                        typeEvent = -1
                  if eventBase.name  == FightEventEnum.FIGHTER_LIFE_GAIN:
                         pass
                  else:
                        typeEvent = 1
                  fightEvent = self.groupByElements(eventsGroupedByTarget[targetEvents],typeEvent,self._detailsActive,deadTargets[eventBase.targetId],eventBase.castingSpellId)
                  ...
                  self.groupByTeam(playerTeamId,targetsId,eventList,entitiesList,deadTargets)
                  ecopy = eventList.copy()
                  for eventBase in ecopy:
                     isLastDamage = eventBase.name == FightEventEnum.FIGHTER_LIFE_LOSS and lastDamageMap[eventBase.targetId] == eventBase.id and deadTargets[eventBase.targetId]
                     eventList.remove(eventBase)
                  ecopy = None
            if groupedByElementsEventList and len(groupedByElementsEventList) > 0:
               self.groupByTeam(playerTeamId,targetsId,groupedByElementsEventList,entitiesList,deadTargets,False)
   ...
   def getDeadTargetsDescr(self) -> object:
      ...
      for eventList in events:
         for fightEvent in eventList:
            if fightEvent.name == FightEventEnum.FIGHTER_LIFE_LOSS and not deadTargets[fightEvent.targetId]:
   ...
   def groupByElements(self, pvgroup:list[FightEvent], pType:int, activeDetails:bool = True, pAddDeathInTheSameMsg:bool = False, pCastingSpellId:int = -1) -> FightEvent:
      ttptsStr:str = ""
      for fe in pvgroup:
      ...
      return FightEvent(fightEventName, newparams,newparams[0], 2, pvgroup[0].castingSpellId, len(pvgroup), 1)
   
   def groupByTeam(self, playerTeamId:int, targets:list[float], pEventList:list[FightEvent], pEntitiesList:dict, deadTargets:dict, pEnableColoration:bool = True) -> bool:
      if len(pEventList) == 0:
         return False
      ...
      while tmpEventList:
         ...
         for event in listToConcat:
            ...
            if event != listToConcat[0] and event.targetId == listToConcat[0].targetId and listToConcat[0] is StatBuff:
            ...
            if isinstance(tmpEffect, EffectInstanceDice):
               ...
               for event in listToConcat:
                  if event.targetId == evt.targetId and event.buff.id is not evt.buff.id:
                     if evt.buff.castingSpell.spellRank.maxStack and buffStackCount >= evt.buff.castingSpell.spellRank.maxStack:
                        break
                     buffStackCount += 1
                     tmpDiceNum += int(event.buff.rawParam1)
               EffectInstanceDice(tmpEffect).diceNum = tmpDiceNum
            evt.params[1] = tmpEffect.description
         team = self.groupEntitiesByTeam(playerTeamId,elist,pEntitiesList, pEventList[0].name not in self.SKIP_ENTITY_ALIVE_CHECK_EVENTS)
            ...
            logger.warn("Failed to group FightEvents for the team \'none\'")
               ...
               for t in pEntitiesList:
                  if isinstance(t, GameFightFighterInformations):
                     ...
                     teamId = t.teamId
                  if team.find("allies") != -1 and teamId == playerTeamId or team.find("enemies") != -1 and teamId != playerTeamId:
   ...
   def getGroupedListEvent(self, pInEventList:list[FightEvent]) -> list[FightEvent]:
      ...
      for event in pInEventList:
         if listToConcat.find(event) == -1 and self.needToGroupFightEventsData(self.getNumberOfParametersToCheck(baseEvent),event,baseEvent):
   ...
   def groupEntitiesByTeam(self, playerTeamId:int, targetList:list[float], entitiesList:dict, checkAlive:bool = True) -> str:
      ...
      for fighterInfos in entitiesList:
         if fighterInfos != None:
            if fighterInfos.spawnInfo.teamId == playerTeamId:
               ...
               nbTotalEnemies += 1
            if (not checkAlive or checkAlive and fighterInfos.spawnInfo.alive) and targetList.find(fighterInfos.contextualId) != -1:
               if fighterInfos.spawnInfo.teamId == playerTeamId:
                  alliesCount += 1
               else:
                  enemiesCount += 1
      ...
      if alliesCount > 1 and alliesCount == nbTotalAllies:
         returnData += (v if returnData != "" else "") + "allies"
   ...
   def needToGroupFightEventsData(self, pNbParams:int, pFightEvent:FightEvent, pBaseEvent:FightEvent) -> bool:
      if pFightEvent.castingSpellId != pBaseEvent.castingSpellId or GameDebugManager().detailedFightLog_unGroupEffects or pFightEvent.buff and pFightEvent.buff.disabled is not pBaseEvent.buff.disabled:
         return False
