...
from com.ankamagames.dofus.logic.game.common.actions.roleplay.AnomalySubareaInformationRequestAction import AnomalySubareaInformationRequestAction
...
from com.ankamagames.dofus.network.messages.game.context.roleplay.AnomalySubareaInformationRequestMessage import AnomalySubareaInformationRequestMessage
...
class WorldFrame(Frame):
   ...
   def getMonsterXpBoostMultiplier(self, pMonsterId:int) -> float:
      ...
      return (boost.xpBoost / 100 if not boost else 1) * self.getRaceXpBoostMultiplier(Monster.getMonsterById(pMonsterId).race)
   
   def getMonsterDropBoostMultiplier(self, pMonsterId:int) -> float:
      boost:MonsterBoosts = self.getMonsterBoost(pMonsterId)
      return (boost.dropBoost / 100 if not boost else 1) * self.getRaceDropBoostMultiplier(Monster.getMonsterById(pMonsterId).race)
