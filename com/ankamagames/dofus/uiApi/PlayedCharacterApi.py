...
from com.ankamagames.dofus.internalDatacenter.items.idolsPresetWrapper import IdolsPresetWrapper
...
from com.ankamagames.dofus.network.types.game.context.roleplay.GameRolePlayMutantInformations import GameRolePlayMutantInformations
from com.ankamagames.dofus.network.types.game.context.roleplay.GuildInformations import GuildInformations
from com.ankamagames.dofus.network.types.game.context.roleplay.HumanOptionTitle import HumanOptionTitle
...
class PlayedCharacterApi(IApi, metaclass=Singleton):
   ...
   def getKamasMaxLimit(self) -> float:
      playedCharacterFrame:PlayedCharacterUpdatesFrame = Kernel().getWorker().getFrame(PlayedCharacterUpdatesFrame)
   ...
   def isInPreFight(self) -> bool:
      return Kernel().getWorker().contains(FightPreparationFrame) or Kernel().getWorker().isBeingAdded(FightPreparationFrame)
   ...
   def getSkin(self) -> int:
      i:CharacterBaseInformations = PlayedCharacterManager().infos
      if EntityLookAdapter.fromNetwork(i.entityLook) and EntityLookAdapter.fromNetwork(i.entityLook).getSkins() and EntityLookAdapter.fromNetwork(i.entityLook).getSkins().length > 0:
