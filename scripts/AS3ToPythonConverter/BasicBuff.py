...
from com.ankamagames.dofus.network.enums.FightDispellableEnum import FightDispellableEnum
...
class BasicBuff:
   ...
   def __init__(self, effect:AbstractFightDispellableEffect = None, castingSpell:CastingSpell = None, actionId:int = 0, param1 = None, param2 = None, param3 = None):
      ...
      if Kernel.beingInReconection or not isPlayerId or fighterInfo is not None and not fighterInfo.spawnInfo.alive:
      ...
      if GameDebugManager().buffsDebugActivated:
         logger.debug("[BUFFS DEBUG] Buff " + self.id + " cr�� ! Params " + param1 + ", " + param2 + ", " + param3 + ", aliveSource " + self.aliveSource + "     \'" + self._effect.description + "\'")
      ...
      for slId in self.castingSpell.spell.spellLevels:
         ...
         if sl:
            ...
            if foundEi:
               ...
               self._effect.visibleInFightLog = True if forceBuffsShowInFightLog else bool(foundEi.visibleInFightLog)
   ...
   def canBeDispell(self, forceUndispellable:bool = False, targetBuffId:int = -2147483648, dying:bool = False) -> bool:
   ...
   def dispellableByDeath(self) -> bool:
      return self.dispelable == FightDispellableEnum.DISPELLABLE_BY_DEATH or self.dispelable == FightDispellableEnum.DISPELLABLE
   ...
   def equals(self, other:BasicBuff, ignoreSpell:bool = False) -> bool:
      sb1:StateBuff = None
      sb2:StateBuff = None
      if self.targetId != other.targetId or self.aliveSource != other.aliveSource or self.effect.effectId != other.actionId or self.duration != other.duration or self.effect.hasOwnProperty("delay") and other.effect.hasOwnProperty("delay") and self.effect.delay != other.effect.delay or self.castingSpell.spellRank and other.castingSpell.spellRank and not ignoreSpell and self.castingSpell.spellRank.id != other.castingSpell.spellRank.id or not ignoreSpell and self.castingSpell.spell.id != other.castingSpell.spell.id or getQualifiedClassName(self) != getQualifiedClassName(other) or self.source != other.source or self.trigger and (self.effect.triggers.find("|") == -1 or self.dataUid != other.dataUid):
         return False
      if self.actionId == ActionIds.ACTION_CHARACTER_PUNISHMENT:
         if self.param1 != other.param1:
            return False
      elif self.actionId == ActionIds.ACTION_BOOST_SPELL_RANGE_MAX or self.actionId == ActionIds.ACTION_BOOST_SPELL_RANGE_MIN or self.actionId == ActionIds.ACTION_BOOST_SPELL_RANGEABLE or self.actionId == ActionIds.ACTION_BOOST_SPELL_DMG or self.actionId == ActionIds.ACTION_BOOST_SPELL_HEAL or self.actionId == ActionIds.ACTION_BOOST_SPELL_AP_COST or self.actionId == ActionIds.ACTION_DEBOOST_SPELL_AP_COST or self.actionId == ActionIds.ACTION_BOOST_SPELL_CAST_INTVL or self.actionId == ActionIds.ACTION_BOOST_SPELL_CC or self.actionId == ActionIds.ACTION_BOOST_SPELL_CASTOUTLINE or self.actionId == ActionIds.ACTION_BOOST_SPELL_NOLINEOFSIGHT or self.actionId == ActionIds.ACTION_BOOST_SPELL_MAXPERTURN or self.actionId == ActionIds.ACTION_BOOST_SPELL_MAXPERTARGET or self.actionId == ActionIds.ACTION_BOOST_SPELL_CAST_INTVL_SET or self.actionId == ActionIds.ACTION_BOOST_SPELL_BASE_DMG or self.actionId == ActionIds.ACTION_DEBOOST_SPELL_RANGE_MAX or self.actionId == ActionIds.ACTION_DEBOOST_SPELL_RANGE_MIN or self.actionId == ActionIds.ACTION_CHARACTER_DISPELL_SPELL or self.actionId == ActionIds.ACTION_TARGET_EXECUTE_SPELL or self.actionId == ActionIds.ACTION_TARGET_EXECUTE_SPELL_WITH_ANIMATION or self.actionId == ActionIds.ACTION_TARGET_EXECUTE_SPELL_ON_SOURCE or self.actionId == ActionIds.ACTION_SOURCE_EXECUTE_SPELL_ON_TARGET or self.actionId == ActionIds.ACTION_SOURCE_EXECUTE_SPELL_ON_SOURCE or self.actionId == ActionIds.ACTION_CHARACTER_ADD_SPELL_COOLDOWN or self.actionId == ActionIds.ACTION_CHARACTER_REMOVE_SPELL_COOLDOWN or self.actionId == ActionIds.ACTION_CHARACTER_PROTECTION_FROM_SPELL or self.actionId == ActionIds.ACTION_CHARACTER_SET_SPELL_COOLDOWN:
         if self.param1 != other.param1:
            return False
      else:
         ...
         if self.actionId == ActionIds.ACTION_CHARACTER_ADD_APPEARANCE:
            if self.dataUid != other.dataUid:
               return False
         elif self.actionId == other.actionId and (self.actionId == ActionIds.ACTION_FIGHT_DISABLE_STATE or self.actionId == ActionIds.ACTION_FIGHT_UNSET_STATE or self.actionId == ActionIds.ACTION_FIGHT_SET_STATE):
            sb1 = self
            sb2 = other
            if sb1 and sb2:
   ...
   def add(self, buff:BasicBuff) -> None:
      ...
      additionDetails:str = ""
      if self.actionId in [ActionIds.ACTION_BOOST_SPELL_BASE_DMG, 
            ...
            additionDetails += "\rparam2 : " + self.param2 + " � " + (self.param2 + buff.param2)
            additionDetails += "\rparam3 : " + self.param3 + " � " + (self.param3 + buff.param3)
            self.param1 = buff.param1
            self.param2 += buff.param2
            self.param3 += buff.param3
      if self.actionId  == ActionIds.ACTION_CHARACTER_PUNISHMENT:
         additionDetails += "\rparam1 : " + self.param1 + " � " + (self.param1 + buff.param2)
         self.param1 += buff.param2
         
      if self.actionId  in [ActionIds.ACTION_FIGHT_SET_STATE, ActionIds.ACTION_FIGHT_UNSET_STATE, ActionIds.ACTION_FIGHT_DISABLE_STATE]:
         if isinstance(self, StateBuff) and isinstance(buff, StateBuff):
            additionDetails += "\rdelta : " + self.delta + " � " + str(self.delta + buff)
            self.delta += buff.delta
            
      else:
            additionDetails += "\rparam1 : " + self.param1 + " � " + (self.param1 + buff.param1)
            additionDetails += "\rparam2 : " + self.param2 + " � " + (self.param2 + buff.param2)
            additionDetails += "\rparam3 : " + self.param3 + " � " + (self.param3 + buff.param3)
            self.param1 += buff.param1
            self.param2 += buff.param2
   ...
   def updateParam(self, value1:int = 0, value2:int = 0, value3:int = 0, buffId:int = -1) -> None:
      ...
      if buffId == -1:
         ...
         if self.id == buffId:
            ...
            i = 0
            while True: 
               j = self.stack.hasnext(j,i)
               p1 += stackBuff.param1
               p2 += stackBuff.param2
               p3 += stackBuff.param3)
               stackBuff = nextvalue(i, j)
               if stackBuff.id != buffId:
                  continue
               if stackBuff.actionId  in [ActionIds.ACTION_CHARACTER_PUNISHMENT, ActionIds.ACTION_FIGHT_SET_STATE, 
                                          ActionIds.ACTION_FIGHT_UNSET_STATE, ActionIds.ACTION_FIGHT_DISABLE_STATE]:
                  break
               else:
                  stackBuff.param1 = value1
                  stackBuff.param2 = value2
      ...
      if self.actionId  == ActionIds.ACTION_CHARACTER_PUNISHMENT:
         self.param1 = p2
      if self.actionId  == ActionIds.ACTION_FIGHT_SET_STATE:
      if self.actionId  == ActionIds.ACTION_FIGHT_UNSET_STATE:
      if self.actionId  == ActionIds.ACTION_FIGHT_DISABLE_STATE:
      else:
            self.param1 = p1
            self.param2 = p2
            self.param3 = p3
      if GameDebugManager().buffsDebugActivated:
         logger.debug("[BUFFS DEBUG] Buff " + self.id + " rafraichissement des params " + self.param1 + ", " + self.param2 + ", " + self.param3)
      self.refreshDescription()
   ...
   def incrementDuration(self, delta:int, dispellEffect:bool = False) -> bool:
      ...
      if GameDebugManager().buffsDebugActivated:
         if dispellEffect:
            logger.debug("[BUFFS DEBUG] Buff " + self.id + " dur�e modifi�e de " + delta + " (desenvoutement de l\'effet)")
      if not dispellEffect or self.canBeDispell():
         ...
         if self.duration + delta > 0:
            ...
            if GameDebugManager().buffsDebugActivated:
               logger.debug("[BUFFS DEBUG] Buff " + self.id + " dur�e modifi�e de " + oldDuration + " � " + self.duration)
            return True
         if self.duration > 0:
            self.duration = 0
   ...
   def __str__(self) -> str:
      return "[BasicBuff id=" + self.id + ", uid=" + self.uid + ", targetId=" + self.targetId + ", sourceId=" + self.source + ", duration=" + self.duration + ", len(stack)=" + (!not self.stack ? len(self.stack) : "0") + "]"
