...
class UnknownMsgId(Exception):
    pass

class UnknownTypeId(Exception):
    pass
...
class DofusProtocol:
    ...
    def getTypeById(self, id):
        if str(id) not in self.json["type_by_id"]:
            raise UnknownTypeId(f"Type id {id} not found in known types ids")
        return self.json["type_by_id"][str(id)]
    
    def getMsgById(self, id):
        if str(id) not in self.json["msg_by_id"]:
            raise UnknownMsgId(f"msg id {id} not found in known msg ids")
