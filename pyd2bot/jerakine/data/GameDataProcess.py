...
from pyd2bot.jerakine.data import I18nFileAccessor
...
class GameDataProcess:
   ...
   def getReadFunctionType(self, type:int) -> FunctionType:
      if type == GameDataTypeEnum.INT:
         ...
         if not (self._stream is Bytelist):
   ...
   def parseStream(self) -> None:
      ...
      indexSearchOffset:int = self._stream.position + fieldListSize + 4
      while fieldListSize:
         size = self._stream.bytesAvailable
         ...
         fieldListSize -= size - self._stream.bytesAvailable
