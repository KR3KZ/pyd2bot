import math
from pyd2bot.gameData.world.mapPoint import Point
...
class MapTools:
    ...
    def getCellsIdBetween(param1:int, param2:int) -> list:
        ...
        if not MapTools.isValidCellId(param1) or not MapTools.isValidCellId(param2):
            return []
         
        _loc3_:int = math.floor(param1 / MapTools.MAP_GRID_WIDTH)
        _loc4_:int = math.floor((_loc3_ + 1) / 2)
        _loc5_ = param1 - _loc3_ * MapTools.MAP_GRID_WIDTH
        _loc6_ = _loc4_ + _loc5_
        _loc7_:int = math.floor(param1 / MapTools.MAP_GRID_WIDTH)
        _loc8_:int = math.floor((_loc7_ + 1) / 2)
        _loc9_ = _loc7_ - _loc8_
        _loc10_ = param1 - _loc7_ * MapTools.MAP_GRID_WIDTH
        _loc11_ = _loc10_ - _loc9_
        _loc12_:int = math.floor(param2 / MapTools.MAP_GRID_WIDTH)
        _loc13_:int = math.floor((_loc12_ + 1) / 2)
        _loc14_ = param2 - _loc12_ * MapTools.MAP_GRID_WIDTH
        _loc15_ = _loc13_ + _loc14_
        _loc16_:int = math.floor(param2 / MapTools.MAP_GRID_WIDTH)
        _loc17_:int = math.floor((_loc16_ + 1) / 2)
        _loc18_ = _loc16_ - _loc17_
        _loc19_ = param2 - _loc16_ * MapTools.MAP_GRID_WIDTH
        ...
        while _loc6_ != _loc15_ or _loc11_ != _loc20_:
            if MapTools.floatAlmostEquals(_loc30_, _loc31_):
                ...
                _loc31_ += _loc27_
                _loc11_ += _loc29_
            _loc32_.append(int(MapTools.getCellIdByCoord(_loc6_, _loc11_)))
        return _loc32_

    def isValidCellId(param1:int) -> bool:
        if not MapTools.isInit:
    ...
    def getCellIdByCoord(param1:int, param2:int) -> int:
        if not MapTools.isValidCoord(param1,param2):
            return -1
        return int(math.floor(float((param1 - param2) * MapTools.MAP_GRID_WIDTH + param2 + (param1 - param2) / 2)))
    
    def isValidCoord(param1:int, param2:int) -> bool:
        if not MapTools.isInit:
            raise Exception("MapTools must be initiliazed with method .initForDofus2 or .initForDofus3")

        if param2 >= -param1 and param2 <= param1 and param2 <= MapTools.MAP_GRID_WIDTH + MapTools.MAX_Y_COORD - param1:
            return param2 >= param1 - (MapTools.MAP_GRID_HEIGHT - MapTools.MIN_Y_COORD)
         
        return False

    def getCellCoordById(param1:int) -> Point:
        if not MapTools.isValidCellId(param1):
            return None
        _loc2_:int = math.floor(param1 / MapTools.MAP_GRID_WIDTH)
        _loc3_:int = math.floor((_loc2_ + 1) / 2)
        _loc4_ = _loc2_ - _loc3_
        _loc5_ = param1 - _loc2_ * MapTools.MAP_GRID_WIDTH
        return Point(_loc3_ + _loc5_, _loc5_ - _loc4_)

    def getCellsCoordBetween(param1:int, param2:int) -> list[Point]:
        cellsIds = MapTools.getCellsIdBetween(param1, param2)
        return [MapTools.getCellCoordById(cellid) for cellid in cellsIds]