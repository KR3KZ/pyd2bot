...
def getCell(mapData:Map, spellrange:list[MapPoint], refPosition:MapPoint) -> list[int]: 
    ...
    for i in range(len(orderedCell)):
        ...
        if tested.get(f"{p.x}_{p.y}") is None or refPosition.x + refPosition.y == p.x + p.y or refPosition.x - refPosition.y == p.x - p.y:
